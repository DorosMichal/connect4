...
def generate_all_connect4():
    ...
    # diagonal right up
    d1 = [
        tuple((i - k, j + k) for k in range(4)) for i in range(3, 7) for j in range(4)
    ]
    # diagonal left up
    d2 = [
        tuple((i - k, j - k) for k in range(4))
        for i in range(3, 7)
        for j in range(3, 7)
    ]
    t = h + v + d1 + d2
...
class Connect4State:
    ...
    def get_legal_moves_mask(self):
        return np.array([self._board_ctr[i] >= 0 for i in range(7)])

    def _scheme_wins(self, scheme, player_no):  # player_no is 0 or 1
        if all(self.board[scheme] == Connect4State.scores[player_no]):
            return player_no
        return -1
